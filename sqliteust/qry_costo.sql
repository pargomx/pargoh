WITH RECURSIVE arbol(historia_id, padre_id, nivel, posicion, titulo, prioridad, completada, tiempo_estimado, tiempo_real) AS (

	SELECT h.historia_id, n.padre_id, n.nivel, n.posicion, h.titulo, h.prioridad, h.completada,
		coalesce((SELECT SUM(t.tiempo_estimado) FROM tareas t WHERE t.historia_id = n.nodo_id),0),
		coalesce((SELECT SUM(t.tiempo_real) FROM tareas t WHERE t.historia_id = n.nodo_id),0)
	FROM nodos n
