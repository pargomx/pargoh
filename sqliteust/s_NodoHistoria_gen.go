...
const columnasNodoHistoria string = "his.historia_id, his.proyecto_id AS proyecto_id, his.persona_id AS persona_id, his.titulo, his.objetivo, his.prioridad, his.completada, coalesce(nod.padre_id, 0), coalesce(nod.padre_tbl, ''), coalesce(nod.nivel, 0), coalesce(nod.posicion, 0), his.minutos_estimado, coalesce((SELECT COUNT(nodo_id) FROM nodos WHERE padre_id = his.historia_id), 0) AS num_historias, coalesce((SELECT COUNT(tarea_id) FROM tareas WHERE historia_id = his.historia_id), 0) AS num_tareas, coalesce((SELECT sum(unixepoch(coalesce(nullif(interv.fin,''),datetime('now','-6 hours'))) - unixepoch(interv.inicio)) FROM intervalos interv JOIN tareas tar ON tar.tarea_id = interv.tarea_id WHERE tar.historia_id = his.historia_id GROUP BY tar.historia_id ), 0) AS segundos"
...
func (s *Repositorio) scanRowNodoHistoria(row *sql.Row, nhist *ust.NodoHistoria) error {
	err := row.Scan(
		&nhist.HistoriaID, &nhist.ProyectoID, &nhist.PersonaID, &nhist.Titulo, &nhist.Objetivo, &nhist.Prioridad, &nhist.Completada, &nhist.PadreID, &nhist.PadreTbl, &nhist.Nivel, &nhist.Posicion, &nhist.MinutosEstimado, &nhist.NumHistorias, &nhist.NumTareas, &nhist.Segundos,
	)
	if err != nil {
